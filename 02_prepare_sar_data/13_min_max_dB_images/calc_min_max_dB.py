...
class CalcMinMaxdB(PBPTQProcessTool):
    ...
    def do_processing(self, **kwargs):
        ...
        if len(self.params['sar_imgs']) == 1:
            ...
            for sar_img in self.params['sar_imgs']:
            ...
            rsgislib.imagecalc.calcMultiImgBandStats(vrt_band_sub_imgs, self.params['out_min_dB_img'], rsgislib.SUMTYPE_MIN, "KEA", rsgislib.TYPE_16INT, 32767, True)
            rsgislib.imageutils.popImageStats(self.params['out_min_dB_img'], usenodataval=True, nodataval=32767, calcpyramids=True)

            rsgislib.imagecalc.calcMultiImgBandStats(vrt_band_sub_imgs, self.params['out_max_dB_img'], rsgislib.SUMTYPE_MAX, "KEA", rsgislib.TYPE_16INT, 32767, True)
            rsgislib.imageutils.popImageStats(self.params['out_max_dB_img'], usenodataval=True, nodataval=32767, calcpyramids=True)
    ...
    def required_fields(self, **kwargs):
        return ["tile", "sar_imgs", "out_min_dB_img", "out_max_dB_img", "out_diff_dB_img", "tmp_dir"]
