...
class CreateVectorTile(PBPTQProcessTool):
    ...
    def do_processing(self, **kwargs):
        pxl_count = rsgislib.imagecalc.count_pxls_of_val(
            self.params["img_tile"], vals=[1]
        )
        print("N Pixels: ", pxl_count[0])

        if pxl_count[0] > 0:
            rsgislib.vectorutils.createvectors.polygonise_raster_to_vec_lyr(
                out_vec_file=self.params["out_vec"],
                out_vec_lyr=self.params["out_lyr_name"],
                out_format="GPKG",
                input_img=self.params["img_tile"],
                img_band=1,
