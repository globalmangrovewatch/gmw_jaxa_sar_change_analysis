...
class CreateImageTile(PBPTQProcessTool):
    ...
    def do_processing(self, **kwargs):
        ...
        band_defns = [rsgislib.imagecalc.BandDefn('po_chng', self.params['potent_chng_msk_img'], 1),
                      rsgislib.imagecalc.BandDefn('mng_chg', self.params['mng_chng_img'], 1),
                      rsgislib.imagecalc.BandDefn('n_mng_chg', self.params['nmng_chng_img'], 1)]
        rsgislib.imagecalc.bandMath(self.params['out_gmw_pchg_msk'], '(po_chng==0)&&(mng_chg==1)?1:(po_chng==1)&&(n_mng_chg==1)?0:po_chng', 'KEA', rsgislib.TYPE_8UINT, band_defns)
        rsgislib.rastergis.populateStats(self.params['out_gmw_pchg_msk'], addclrtab=True, calcpyramids=True, ignorezero=True)

        if os.path.exists(self.params['tmp_dir']):
            shutil.rmtree(self.params['tmp_dir'])

    def required_fields(self, **kwargs):
        return ["tile", "gmw_tile", "year", "potent_chng_msk_img", "mng_chng_img", "nmng_chng_img", "out_gmw_mng_msk", "out_gmw_pchg_msk", "tmp_dir"]

    def outputs_present(self, **kwargs):
        files_dict = dict()
        files_dict[self.params['out_gmw_mng_msk']] = 'gdal_image'
        files_dict[self.params['out_gmw_pchg_msk']] = 'gdal_image'
    ...
    def remove_outputs(self, **kwargs):
        ...
        if os.path.exists(self.params['out_gmw_pchg_msk']):
            os.remove(self.params['out_gmw_pchg_msk'])
