...
class GenCmds(PBPTGenQProcessToolCmds):
    ...
    def gen_command_info(self, **kwargs):
        ...
        for gmw_tile in img_tiles:
            ...
            out_mng_chng_rmsml_img = os.path.join(kwargs['out_dir'], '{}_{}_mng_chng.kea'.format(tile_basename, kwargs['sar_year']))
            out_nmng_chng_rmsml_img = os.path.join(kwargs['out_dir'], '{}_{}_not_mng_chng.kea'.format(tile_basename, kwargs['sar_year']))

            if not os.path.exists(out_mng_chng_rmsml_img):
                print("rm {}".format(mng_chng_img))
                c_dict = dict()
                c_dict['tile'] = tile_basename
                c_dict['chng_img'] = mng_chng_img
                c_dict['out_img'] = out_mng_chng_rmsml_img
                c_dict['tmp_dir'] = os.path.join(kwargs['tmp_dir'], "{}_{}_rmsml_mng_chng".format(tile_basename, kwargs['sar_year']))
                if not os.path.exists(c_dict['tmp_dir']):
                    os.mkdir(c_dict['tmp_dir'])
                self.params.append(c_dict)

            if not os.path.exists(out_nmng_chng_rmsml_img):
                print("rm {}".format(nmng_chng_img))
                c_dict = dict()
                c_dict['tile'] = tile_basename
                c_dict['chng_img'] = nmng_chng_img
                c_dict['out_img'] = out_nmng_chng_rmsml_img
    ...
    def run_gen_commands(self):
        ...
        for year in ['1996', '2007', '2008', '2009', '2015', '2016', '2017', '2018', '2019', '2020']:
            self.gen_command_info(gmw_tiles='/scratch/a.pfb/gmw_v3_change/data/gmw_baseline/gmw_2010_v3/*.kea',
                                  ...
                                  sar_year=year,
                                  out_dir='/scratch/a.pfb/gmw_v3_change/data/gmw_chng_data/gmw_2010_{}_chngs_rmsml'.format(year),
                                  tmp_dir='/scratch/a.pfb/gmw_v3_change/tmp')
        
        self.pop_params_db()
        self.create_slurm_sub_sh("gmw_rmsml_chng", 16448, '/scratch/a.pfb/gmw_v3_change/logs',
