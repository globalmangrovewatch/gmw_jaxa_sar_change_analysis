...
for roi_id in roi_ids:
    ...
    for base_year in gmw_years:
        ...
        base_img = os.path.join(f"../00_data/01_site_maps/{base_year}", f"gmw_{base_year}_v312_site_{roi_id}.kea")
        for chng_year in gmw_years:
            print(f"\t\tChange Year: {chng_year}")
            chng_img = os.path.join(f"../00_data/01_site_maps/{chng_year}", f"gmw_{chng_year}_v312_site_{roi_id}.kea")
