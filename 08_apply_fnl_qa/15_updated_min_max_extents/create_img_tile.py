...
class CreateImageTile(PBPTQProcessTool):
    ...
    def do_processing(self, **kwargs):
        ...
        rsgislib.imagecalc.band_math(self.params['out_min_ext_img'], '(gmw==0)&&(min==1)?0:min', 'KEA', rsgislib.TYPE_8UINT, band_defns)
        rsgislib.rastergis.pop_rat_img_stats(self.params['out_min_ext_img'], add_clr_tab=True, calc_pyramids=True, ignore_zero=True)

        band_defns = [rsgislib.imagecalc.BandDefn('gmw', self.params['gmw_tile'], 1),
                      rsgislib.imagecalc.BandDefn('max', self.params['max_ext_img'], 1)]
        rsgislib.imagecalc.band_math(self.params['out_max_ext_img'], '(gmw==1)&&(max==0)?1:max', 'KEA', rsgislib.TYPE_8UINT, band_defns)
        rsgislib.rastergis.pop_rat_img_stats(self.params['out_max_ext_img'], add_clr_tab=True, calc_pyramids=True, ignore_zero=True)
