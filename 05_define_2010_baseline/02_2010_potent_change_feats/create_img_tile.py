...
class CreateImageTile(PBPTQProcessTool):
    ...
    def do_processing(self, **kwargs):
        ...
        rsgislib.imagecalc.bandMath(self.params['out_img'], '(gmwinit==1)&&(gmw==0)?1:(pchng==1)&&(wmsk==1)?1:0', 'KEA', rsgislib.TYPE_8UINT, band_defns)
        rsgislib.rastergis.populateStats(self.params['out_img'], addclrtab=True, calcpyramids=True, ignorezero=True)


    def required_fields(self, **kwargs):
        return ["tile", "gmw_tile", "gmw_init_msk_img", "water_msk_img", "pot_chng_rgns_img", "out_img"]
