...
for roi_id in roi_ids:
    ...
    pxl_area_dir = f"../00_data/02_site_pxl_area"
    out_dir = f"../00_data/03_site_chng_maps_clumps/site_{roi_id}"
    ...
    for i, base_year in enumerate(gmw_years):
        ...
        if i < n_years-1:
            ...
            pxl_area_img = os.path.join(pxl_area_dir, f"gmw_chng_site_{roi_id}_pxl_area_ha.kea")
            chng_clumps_img = os.path.join(out_dir, f"gmw_chng_site_{roi_id}_{base_year}_{chng_year}.kea")

            if i == 0:
                # Calculate the pixel area.
                calc_wgs84_pixel_area(input_img=chng_img, output_img=pxl_area_img, scale=10000, gdalformat='KEA')

            rsgislib.segmentation.clump(chng_img, chng_clumps_img, "KEA", False, 0, False)
            rsgislib.rastergis.pop_rat_img_stats(chng_clumps_img, True, True, True)

            rsgislib.rastergis.populate_rat_with_mode(input_img=chng_img, clumps_img=chng_clumps_img, out_cols_name="chng_cls", use_no_data=True, no_data_val=0, out_no_data=True, mode_band=1, rat_band=1)
